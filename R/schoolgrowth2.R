schoolgrowth2 <- function(d, target = NULL, target_contrast = NULL, control = list(), vE = NULL, vX = NULL){
    ...
    ##
    ## FLAG: (schoolgrowth2): we use Ytilde here and estimate only school FE.
    ## Things could be simplified at this point because things break out by school.
    ## need to add back in block*pattern mean contribution that are treated as fixed
    ## #############################################
    if(!mean_supplied){
        ...
        ## "Y"
        ## FLAG YTILDE HERE!
        ## FLAG also tracking "fixedpart" which we will have to add back in as a fixed mean
        Y <- matrix(as.vector(unlist(lapply(dsch, function(x){ x$tab$Ytilde }))), ncol=1)
        stopifnot( (nrow(Y) == N) && all(d$Ytilde[!duplicated(d$sbid)] == Y) )
        fixedpart <- as.vector(tapply(d$muhat_bp, d$sbid, mean))
        stopifnot(max(abs(fixedpart - as.vector(unlist(lapply(dsch, function(x){ x$tab$Y - x$tab$Ytilde}))))) < 1e-7)
        ...
        ## FLAG adding in fixed mean here as well!
        .Xp_Vinv    <- crossprod(.X, Vinv)
        .Xp_Vinv_X  <- as(.Xp_Vinv %*% .X, "symmetricMatrix")
        .bgls       <- solve(.Xp_Vinv_X, (.Xp_Vinv %*% Y))
        .Xbgls      <- .X %*% .bgls
        .blp        <- fixedpart + as.vector(.Xbgls + (G %*% Vinv %*% (Y - .Xbgls)))
        bhat        <- as.vector(.bgls)
        names(bhat) <- colnames(.X)

        ## summaries of BLUPs
        ## FLAG THIS IS OFF BECAUSE WE SHOULD BE LOOKING AT CORRELATION WITH REAL Y NOT YTILDE
